...
describe('content.backend-http provider', () => {
  ...
  it('normalizes legacy code 1 at backend adapter boundary', async () => {
    vi.stubGlobal('uni', {
      request(options: MockUniRequestOptions) {
        requestCalls.push(options)
        options.success?.({
            ...
            data: [
              {
                ...
                min_price: '88.00',
                new_time: '2026-04-24T10:00:00+08:00',
                flux: '9',
              },
            ],
          },
    ...
    expect(response.envelope.code).toBe(0)
    expect(response.envelope.data?.items[0]?.title).toBe('Legacy Success')
  })
  ...
  it('keeps the current market page in backend order without sending unsupported sort fields', async () => {
    vi.stubGlobal('uni', {
      request(options: MockUniRequestOptions) {
        requestCalls.push(options)
        options.success?.({
    ...
    expect(response.envelope.data?.items.map((item) => item.title)).toEqual(['High', 'Low'])
  })

  it('sends blind box market request to marketList with goods_type 1 and keeps backend order', async () => {
    vi.stubGlobal('uni', {
      request(options: MockUniRequestOptions) {
        requestCalls.push(options)
        options.success?.({
    ...
    expect(requestCalls[0]).toMatchObject({
      method: 'POST',
      url: 'https://legacy.example.com/market/market/marketList',
      data: {
        goods_type: 1,
        page: 1,
        list_rows: 10,
    ...
    expect(response.envelope.data?.items.map((item) => item.title)).toEqual(['Low Box', 'High Box'])
  })

  it('keeps loaded collection page in backend order when flux differs', async () => {
    vi.stubGlobal('uni', {
      request(options: MockUniRequestOptions) {
        requestCalls.push(options)
        options.success?.({
    ...
    expect(response.envelope.data?.items.map((item) => item.title)).toEqual([
      'High Issue Low Flux',
      'Low Issue High Flux',
    ])
  })
