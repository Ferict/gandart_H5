...
const backendEndpoints = {
  ...
  marketBoxList: '/market/market/marketList',
...
const isLegacySuccessCode = (code: number): boolean => code === 0 || code === 1
...
const resolveEnvelopeMessage = (raw: LegacyBackendResponse): string => {
  ...
  return isLegacySuccessCode(resolveEnvelopeCode(raw)) ? 'ok' : 'backend error'
}
...
const createContentEnvelope = <T>(
  ...
  const legacyCode = resolveEnvelopeCode(raw)
  return {
    code: isLegacySuccessCode(legacyCode) ? 0 : legacyCode,
...
const findFailedLegacyResponse = (
  ...
  responses.find((response) => !isLegacySuccessCode(resolveEnvelopeCode(response.envelope)))
...
const normalizeLegacyMarketItem = (
  ...
  const payload: ContentMarketItemPayloadDto = {
    ...
    holderCount: Math.max(
      Math.round(firstNumber(item.flux, item.hold_num, item.holderCount) ?? 0),
      0
    ),
  ...
  return {
    ...
    async getResource(input: ContentResourceRequestDto) {
      if (input.resourceType === 'notice') {
        ...
        if (!isLegacySuccessCode(resolveEnvelopeCode(response.envelope))) {
    ...
    async getList(
      ...
      if (input.resourceType === 'market_item') {
        ...
        requestPayload.goods_type = marketKind === 'blindBoxes' ? 1 : 2
        requestPayload.series_id =
          input.categoryId && input.categoryId !== 'all' ? input.categoryId : 0
        if (input.keyword) {
          requestPayload.keywords = input.keyword
        }
        const response = await requestLegacy(
          marketKind === 'blindBoxes'
            ? backendEndpoints.marketBoxList
            : backendEndpoints.marketList,
          requestPayload
        )
        if (!isLegacySuccessCode(resolveEnvelopeCode(response.envelope))) {
          return {
        ...
        return {
          envelope: createContentEnvelope<ContentListDto>(response.envelope, {
            ...
            total: extracted.total ?? normalizedItems.length,
