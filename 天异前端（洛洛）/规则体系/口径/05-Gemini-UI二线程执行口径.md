...
> 状态：active
> 更新时间：2026-04-25
> 说明：本文件用于约束 Gemini UI 转化二线程的工作方式、写入边界、git 分类、停点条件和与主线的交接方式；目标是让二线程按既定形式工作，避免返工、越界和把视觉稿直接打穿到正式契约链，并强制它从项目级后端契约入口读取字段依据。
...
5. `后端契约入口确认`
   - 明确本批已读取 [规则体系/后端契约/README.md](/H:/工作/天异/uniapp+vue新架构/规则体系/后端契约/README.md)
   - 明确本批引用的字段来自 `confirmed`、`待确认` 还是 `inferred` 层
6. `通讯入口确认`
   - 明确本批已读取 [通讯/README.md](/H:/工作/天异/uniapp+vue新架构/通讯/README.md)
   - 明确本批完成或阻塞时会追加写入 [通讯/README.md](/H:/工作/天异/uniapp+vue新架构/通讯/README.md) 的 `消息流`
...
7. 主线通讯只认：
   - [通讯/README.md](/H:/工作/天异/uniapp+vue新架构/通讯/README.md)
8. 只有 `confirmed` 项能当稳定字段依据；`exported-unused`、`unresolved`、`inferred` 只能标记为待确认缺口
...
1. 二线程不得把 `规则体系/后端契约/` 中的待确认项直接写成页面已可依赖的新字段。
2. 若设计稿或外来 UI 需要的字段只落在待确认层，必须在 brief、审查或交付中明确标成“待 `P11.12` 签收”。
3. 二线程判断字段用途时只读 [08-P11.12前端改造口径.md](/H:/工作/天异/uniapp+vue新架构/规则体系/口径/08-P11.12前端改造口径.md)。
4. 二线程判断字段缺口时只读 [09-P11.12需后端补充口径.md](/H:/工作/天异/uniapp+vue新架构/规则体系/口径/09-P11.12需后端补充口径.md)。
5. 二线程不得把后端口径、前端改造后口径、需要后端补充口径和真实 API 接口混写在 brief 或页面代码里。
...
2. `P11.12` 负责现有 UI 适配新版后端契约，并以 `规则体系/后端契约/` 为项目级契约入口
