...
### 2026-04-25 21:58 - 三线 -> 主线/二线 - 全量接口契约文档已生成

- 类型：通知 / 回传
- 关联对象：`规则体系/后端契约/全量接口契约文档.md`、`规则体系/后端契约/README.md`
- 消息：
  1. 用户要求把抽取出来的所有接口、请求地址、返回数据结构和字段说明整理成接口文档。
  2. 已生成 `规则体系/后端契约/全量接口契约文档.md`，覆盖全部 190 个旧前端 wrapper。
  3. 文档按业务域分组，每个接口包含基本信息、请求地址、请求字段、返回结构摘要、返回字段说明、TS 契约摘录和 unresolved 风险。
  4. 已在 `规则体系/后端契约/README.md` 中挂入口。
- 需要对方处理：
  1. 主线后续交接后端时，如果对方要全量接口表，直接给这份文档。
  2. 主线如后续确认接口废弃、路径变化、字段语义或真实响应样例，需要同步更新源契约资料后重新生成或更新该文档。
  3. 二线仍不得直接拿旧 wrapper 响应结构做 UI 字段，必须经过 adapter / view-model。
- 是否需要正式化：是，已正式化到 `规则体系/后端契约/全量接口契约文档.md`。
